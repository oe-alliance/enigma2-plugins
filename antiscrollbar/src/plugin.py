	...
	def openCurrentSeviceConfig(self):
		...
		for mode in config.plugins.antiscrollbar.mode:
			if mode.sref.value == current_ref_str:
				smode = mode
				if mode.sizex.value == 0:
					mode.sizex.value = 200
					mode.sizex.save()
				if mode.sizey.value == 0:
					mode.sizey.value = 200
					mode.sizey.save()
				if mode.posx.value == 0:
					mode.posx.value = 100
					mode.posx.save()
				if mode.posy.value == 0:
					mode.posy.value = 50
					mode.posy.save()
				break
		if smode is None:
			...
			config.plugins.antiscrollbar.mode.append(ConfigSubsection())
			config.plugins.antiscrollbar.mode[i].sref = ConfigText(default=current_ref_str)
			config.plugins.antiscrollbar.mode[i].sizex = ConfigInteger(default=200, limits=(1, 1920))
			config.plugins.antiscrollbar.mode[i].sizey = ConfigInteger(default=200, limits=(1, 1080))
			config.plugins.antiscrollbar.mode[i].posx = ConfigInteger(default=100, limits=(0, 1920))
			config.plugins.antiscrollbar.mode[i].posy = ConfigInteger(default=50, limits=(0, 1080))
			config.plugins.antiscrollbar.mode[i].enabled = ConfigYesNo(default=True)

			config.plugins.antiscrollbar.mode[i].sref.save()
			config.plugins.antiscrollbar.mode[i].sizex.save()
			config.plugins.antiscrollbar.mode[i].sizey.save()
			config.plugins.antiscrollbar.mode[i].posx.save()
			config.plugins.antiscrollbar.mode[i].posy.save()
			config.plugins.antiscrollbar.mode[i].enabled.save()
			config.plugins.antiscrollbar.mode[i].sref.save()
			config.plugins.antiscrollbar.modescount.value = i + 1
