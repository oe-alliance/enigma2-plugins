...
class AutoPollerThread(Thread):
	...
	def start(self, initial=True):
		...
		if initial: delay = 10
		else: delay = config.plugins.autotimer.interval.value*60
	...
	def run(self):
		...
		if (config.plugins.autotimer.onlyinstandby.value and inStandby) or (not config.plugins.autotimer.onlyinstandby.value):
			...
			while 1:
				...
				timer.startLongTimer(config.plugins.autotimer.interval.value*60)
		else:
			timer.startLongTimer(config.plugins.autotimer.interval.value*60)
