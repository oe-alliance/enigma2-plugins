...
//START class EMUList
function EMUList(xml){
	// parsing values from xml-element
	debug('init EMUList'+xml);
	try{
		this.xmlitems = xml.getElementsByTagName("e2emulist").item(0).getElementsByTagName("e2emu");
	} catch (e) {
		debug("EMUList parsing Error" + e);
	}
	this.getArray = function(){
		var listxy = new Array();
		for(var i=0;i<this.xmlitems.length;i++){
			//debug("parsing movie "+i+" of "+this.xmlitems.length);
			var xv = new EMU(this.xmlitems.item(i));
			listxy.push(xv);			
		}
		return listxy;
	}
...
function EMU(xml){	
	...
	//debug('init Movie');
	try{
		this.file = xml.getElementsByTagName('e2file').item(0).firstChild.data;
	} catch (e) {
		this.file = "N/A";
	}
	try{
	...
	try{
		this.status = xml.getElementsByTagName('e2status').item(0).firstChild.data;
	} catch (e) {
		this.status = "not running";
	}
	try{
		this.link = xml.getElementsByTagName('e2link').item(0).firstChild.data;
	} catch (e) {
		this.link = "";
	}
