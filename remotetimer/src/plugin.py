...
def localGetPage(url):
    ...
    if username and password:
        ...
        AuthHeaders = {b"Authorization": b"Basic %s" % base64string}
    else:
        AuthHeaders = {}
    return deferPage(url, headers=AuthHeaders)
...
#        try:
        ...
        if r.status_code == 200:
            ...
            errormsg = "[remotetimer][getPage] incorrect response: %d" % r.status_code
            errback(errormsg)
    ...
    if not self.timerentry_remote.value:
        ...
        if config.plugins.remoteTimer.remotedir.value:
            if PY2:
                ...
                rt_dirname = quote(self.timerentry_dirname.value.encode('utf8', 'ignore'))
        else:
            rt_dirname = "None"
