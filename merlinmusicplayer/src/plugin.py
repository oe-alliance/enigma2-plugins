                        ...
                        if row is None:
                            ...
                            if audio:
                                ...
                                if row is None:
                                    ...
                                    genreID = row[0]
                                try:
                                    cursor.execute('INSERT INTO Songs (filename,title,artist_id,album_id,genre_id,tracknumber, bitrate, length, track, date) VALUES(?,?,?,?,?,?,?,?,?,?);', (os_path.join(root, filename),
                                     title,
                                     artistID,
                                     albumID,
                                     genreID,
...
class iDreamAddToDatabase(Screen):
    ...
    def __init__(self, session, initDir):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'back': self.cancel,
         'green': self.green,
         'red': self.cancel,
         'ok': self.green}, -1)
...
class myHTTPClientFactory(HTTPClientFactory):
    ...
    def __init__(self, url, method = 'GET', postdata = None, headers = None, agent = 'SHOUTcast', timeout = 0, cookies = None, followRedirect = 1, lastModified = None, etag = None):
...
class MerlinMusicPlayerScreenSaver(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions',
         'DirectionActions',
         'ColorActions',
         'EventViewActions'], {'back': self.close,
         'right': self.close,
         'left': self.close,
         'up': self.close,
         'down': self.close,
...
class MerlinMusicPlayerTV(MerlinMusicPlayerScreenSaver):
    ...
    def __init__(self, session, currentService, servicelist):
        ...
        self['actions'] = ActionMap(['OkCancelActions',
         ...
         'showEPGList': self.openEventView}, -1)
        self['actions2'] = NumberActionMap(['NumberActions'], {'1': self.keyNumberGlobal,
...
class MerlinMusicPlayerScreen(Screen, InfoBarBase, InfoBarSeek, InfoBarNotifications):
    ...
    def __init__(self, session, songlist, index, idreammode, currentservice, servicelist):
        ...
        self['actions'] = ActionMap(['WizardActions',
         ...
         'ColorActions'], {'back': self.closePlayer,
        ...
        if config.plugins.merlinmusicplayer.startlastsonglist.value:
            ...
            if connection is not None:
                ...
                for song in self.origSongList:
                    cursor.execute('INSERT INTO CurrentSongList (song_id, filename,title,artist,album,genre, bitrate, length, track, date, PTS) VALUES(?,?,?,?,?,?,?,?,?,?,?);', (song[0].songID,
                     song[0].filename,
                     song[0].title,
                     song[0].artist,
                     song[0].album,
    ...
    def coverDownload(self, urls, album, title):
        ...
        if title != '' and title != 'n/a':
            filename = re.sub('[^A-Za-z0-9_-]', '', title) + '_' + parts[-1]
        else:
            filename = re.sub('[^A-Za-z0-9_-]', '', album) + '_' + parts[-1]
    ...
    def getNextTitle(self):
        ...
        elif self.currentIndex + 1 > len(self.songList) - 1:
            index = 0
        else:
        ...
        elif self.songList[index][0].filename.lower().startswith('http://'):
...
class iDreamMerlin(Screen):
    ...
    def __init__(self, session, servicelist):
        ...
        self['actions'] = ActionMap(['WizardActions',
         ...
         'info': self.info_pressed}, -1)
        self['actions2'] = NumberActionMap(['InputActions'], {'0': self.keyNumber_pressed}, -1)
        self.onLayoutFinish.append(self.startRun)
        self.onShown.append(self.lcdUpdate)
        self.onClose.append(self.__onClose)
        self.serviceList = servicelist
...
class MerlinMediaPixmap(Pixmap):
    ...
    def __init__(self):
        ...
        self.coverFileNames = ['folder.png',
         'folder.jpg',
         'cover.jpg',
         'cover.png',
         'coverArt.jpg']
...
class MerlinMusicPlayerFileList(Screen):
    ...
    def __init__(self, session, servicelist):
        ...
        self['actions'] = ActionMap(['WizardActions',
         ...
         'red': self.stopPlayingAndAppendFileToSongList}, -1)
        self.serviceList = servicelist
    ...
    def readCUE(self, filename):
        ...
        import re
        performer_re = re.compile('PERFORMER "(?P<performer>.*?)"(?:=\\r\\n|\\r|\\n|$)')
        title_re = re.compile('TITLE "(?P<title>.*?)"(?:=\\r\\n|\\r|\\n|$)')
        filename_re = re.compile('FILE "(?P<filename>.+?)".*(?:=\\r\\n|\\r|\\n|$)', re.DOTALL)
        track_re = re.compile('TRACK (?P<track_number>[^ ]+?)(?:[ ]+.*?)(?:=\\r\\n|\\r|\\n|$)')
        index_re = re.compile('INDEX (?P<index_nr>[^ ]+?)[ ]+(?P<track_index>[^ ]+?)(?:=\\r\\n|\\r|\\n|$)')
...
def menu_merlinmusicplayerfilelist(menuid, **kwargs):
    if menuid == 'mainmenu' and config.plugins.merlinmusicplayer.merlinmusicplayermainmenu.value:
        return [(_('Merlin Music Player'),
          merlinmusicplayerfilelist,
          'merlin_music_player',
          46)]
    return []
