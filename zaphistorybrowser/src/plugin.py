...
def addToHistory(instance, ref):
    ...
    if instance.servicePath is not None:
        ...
        if history_length > config.plugins.ZapHistoryConfigurator.maxEntries_zap_history.value:
            del instance.history[0]
            history_length -= 1
        instance.history_pos = history_length - 1
        if config.plugins.ZapHistoryConfigurator.e1_like_history.value:
            # TODO: optimize this
            if instance.history == instance.history_tv:
                config.plugins.ZapHistoryConfigurator.history_tv.value = [[entry.toString() for entry in item] for item in instance.history]
            else:
                config.plugins.ZapHistoryConfigurator.history_radio.value = [[entry.toString() for entry in item] for item in instance.history]
...
def newInit(self, session):
    ...
    if config.plugins.ZapHistoryConfigurator.e1_like_history.value:
        append_tv = self.history_tv.append
        for item in config.plugins.ZapHistoryConfigurator.history_tv.value:
            append_tv([eServiceReference(ref) for ref in item])
        append_radio = self.history_radio.append
        for item in config.plugins.ZapHistoryConfigurator.history_radio.value:
            append_radio([eServiceReference(ref) for ref in item])
    ...
    def save(self):
        ...
        if config.plugins.ZapHistoryConfigurator.e1_like_history.value and config.plugins.ZapHistoryConfigurator.e1_like_history.isChanged():
            ...
            try:
                ...
                config.plugins.ZapHistoryConfigurator.history_tv.value = [[entry.toString() for entry in item] for item in csel.history_tv]
                config.plugins.ZapHistoryConfigurator.history_radio.value = [[entry.toString() for entry in item] for item in csel.history_radio]
...
def ZapHistoryBrowserListEntry(serviceName, eventName):
    ...
    if desktop_width <= 1280:
        ...
        result.append(MultiContentEntryText(pos=(x1, y1), size=(w1, h1), font=0, text=serviceName))
        result.append(MultiContentEntryText(pos=(x2, y2), size=(w2, h2), font=1, text=eventName))
    else:
...
class ZapHistoryBrowser(Screen, ProtectedScreen):
    ...
    def buildList(self):
        entries = []
        for item in self.servicelist.history:
            if len(item) == 2:  # Single-Bouquet
                ref = item[1]
            else:  # Multi-Bouquet
                ref = item[2]
            ...
            if info:
                ...
                name = "N/A"
                eventName = ""
            entries.append(ZapHistoryBrowserListEntry(name, eventName))
        entries.reverse()
        self["list"].setList(entries)

    def zap(self):
        history_length = len(self.servicelist.history)
        if history_length > 0:
            self.servicelist.history_pos = (history_length - self["list"].getSelectionIndex()) - 1
            self.servicelist.setHistoryPath()

    def clear(self):
        if self.allowChanges:
            for _ in range(len(self.servicelist.history)):
    ...
    def delete(self):
        if self.allowChanges:
            history_length = len(self.servicelist.history)
            if history_length > 0:
                idx = (history_length - self["list"].getSelectionIndex()) - 1
