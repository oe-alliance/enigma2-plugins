...
int transfer_start(int f_ts, int f_out, off64_t n1, off64_t &n1ret)
{
	...
	if (use_leadin)
	{
		...
		do
		{
			...
			if (read(f_ts, buf, tmp) != tmp)
				return 1;
		} while ((pos = framesearch_b(buf, 0, tmp, -1)) == -1 && num < n1);
		if (pos != -1)
		{
			if (write(f_out, buf + pos, tmp - pos) != tmp - pos)
				return 1;
			n1ret = n1 - (num - pos);
			size_offset -= (num - pos);
			num -= tmp;
			while (num > 0)
			{
				if (read(f_ts, buf, LEN) != LEN)
					return 1;
				if (write(f_out, buf, LEN) != LEN)
					return 1;
				num -= LEN;
			}
		}
...
int transfer_rest(int f_ts, int f_out, off64_t n1, off64_t n2, off64_t &n2ret)
{
	...
	else
	{
		if (i < n2)
		{
			if (read(f_ts, buf, n2 - i) != n2 - i)
				return 1;
			if (write(f_out, buf, n2 - i) != n2 - i)
				return 1;
		}
