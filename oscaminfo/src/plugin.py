...
from Components.config import config, configfile, getConfigListEntry, ConfigSubsection, ConfigDirectory, ConfigText, ConfigPassword, ConfigIP, ConfigInteger, ConfigYesNo
...
config.plugins.oscaminfo = ConfigSubsection()
config.plugins.oscaminfo.userdatafromconf = ConfigYesNo(default = False)
config.plugins.oscaminfo.autoupdate = ConfigYesNo(default = False)
...
class OscamInfoMenu(Screen):
	...
	def __init__(self, session):
		...
		self.menu = [ _("Show /tmp/ecm.info"), _("Show Clients"), _("Show Readers/Proxies"), _("Show Log"), _("Card infos (CCcam-Reader)"), _("ECM Statistics"), _("Setup"), _("Show readme.txt") ]
		...
		if entry == 0:
			...
			if config.plugins.oscaminfo.userdatafromconf.value:
				if self.osc.confPath() is None:
					config.plugins.oscaminfo.userdatafromconf.value = False
					config.plugins.oscaminfo.userdatafromconf.save()
...
class oscReadMe(Screen):
	...
	skin = """<screen name="oscReadMe" position="center,center" size="%s, 500" title="Readme" >
			<widget name="status" position="10,20" size="%s,480" scrollbarMode="showOnDemand"/>
		</screen>""" % ( sw, sw - 10 )
	def __init__(self, session):
		Screen.__init__(self, session)
	...
	def getReadme(self):
		readme_file = resolveFilename(SCOPE_CURRENT_PLUGIN, "Extensions/OscamInfo/readme.txt")
...
class oscECMInfo(Screen, OscamInfo):
	...
	def __init__(self, session):
		...
		if config.plugins.oscaminfo.autoupdate.value:
			self.loop = eTimer()
			self.loop.callback.append(self.showData)
			timeout = config.plugins.oscaminfo.intervall.value * 1000
...
class OscamInfoConfigScreen(Screen, ConfigListScreen):
	...
	def __init__(self, session, msg = None):
		...
		self.createSetup()
		config.plugins.oscaminfo.userdatafromconf.addNotifier(self.elementChanged, initial_call = False)
		config.plugins.oscaminfo.autoupdate.addNotifier(self.elementChanged, initial_call = False)
	...
	def createSetup(self):
		self.oscamconfig = []
		self.oscamconfig.append(getConfigListEntry(_("Read Userdata from oscam.conf"), config.plugins.oscaminfo.userdatafromconf))
		if not config.plugins.oscaminfo.userdatafromconf.value:
			self.oscamconfig.append(getConfigListEntry(_("Username (httpuser)"), config.plugins.oscaminfo.username))
			self.oscamconfig.append(getConfigListEntry(_("Password (httpwd)"), config.plugins.oscaminfo.password))
			self.oscamconfig.append(getConfigListEntry(_("IP-Adress"), config.plugins.oscaminfo.ip))
			self.oscamconfig.append(getConfigListEntry("Port", config.plugins.oscaminfo.port))
		self.oscamconfig.append(getConfigListEntry(_("Automatically update Client/Server View?"), config.plugins.oscaminfo.autoupdate))
		if config.plugins.oscaminfo.autoupdate.value:
			self.oscamconfig.append(getConfigListEntry(_("Update interval (in seconds)"), config.plugins.oscaminfo.intervall))
...
def main(session, **kwargs):
	global HDSKIN, sizeH
	m = session.open(OscamInfoMenu)
						
def Plugins(**kwargs):
	return [PluginDescriptor(name="OscamInfo",
				description=_("Shows information about Oscam Servers"), 
