...
def eject(dev):
    try:
        with open(dev) as cd:
            ioctl(cd.fileno(), int(0x5307))
            ioctl(cd.fileno(), int(0x5329))
            ioctl(cd.fileno(), int(0x5309))
...
class DVDBackup:
    ...
    def gotInfo(self, result, retval, extra_args):
        result = result.decode("utf-8", "ignore") if isinstance(result, bytes) else str(result or "")
        if result and "File Structure DVD" in result and "Main feature:" in result:
            ...
            for line in lines:
                ...
                if len(tmp) == 1:
                    ...
                    if "VTS_00_0." not in name:
                        self.files.append(DVDBackupFile(name, int(size)))
            if len(self.files) > 0:
                if not self.console:
                    self.console = eConsole()
                log = " 2>> /tmp/dvdbackup.log" if config.plugins.DVDBackup.log.value else ""
                cmd = 'dvdbackup -M -v -i %s -o "%s" -n "%s"%s' % (
                    config.plugins.DVDBackup.device.value,
                    config.plugins.DVDBackup.directory.value,
                    config.plugins.DVDBackup.name.value,
                    log,
                )
    ...
    def genisoimageProgress(self, name, data):
        data = data.decode("utf-8", "ignore") if isinstance(data, bytes) else str(data or "")
        if "%" in data:
            for line in data.split("\n"):
                if "%" in line:
                    ...
                    value = value.replace(" ", "")
                    if value:
                        self.genisoimage.progress = int(value)
    ...
    def refreshList(self):
        ...
        for file in dvdbackup.files:
            ...
            if file.progress == 0:
                ...
                items.append(DVDBackupListEntry(file))
        items.extend(tostart)
...
def filescan_open(file_list, session, **kwargs):
    global SESSION
    SESSION = session
    if len(file_list) == 1 and file_list[0].mimetype == "video/x-dvd":
        splitted = file_list[0].path.split('/')
        if len(splitted) > 2 and splitted[1] == 'autofs':
...
def Plugins(**kwargs):
    return [
        PluginDescriptor(name=_("DVD Backup"), description=_("Backup your Video-DVD to your harddisk"), where=[PluginDescriptor.WHERE_EXTENSIONSMENU, PluginDescriptor.WHERE_PLUGINMENU], icon="DVDBackup.png", fnc=main),
